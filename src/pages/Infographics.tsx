...
import {
  ...
  ArrowRight, ArrowLeft, Cpu, Download, Share2, RefreshCw, ThumbsUp, ThumbsDown, HelpCircle, Loader2, X,
} from 'lucide-react'
import WorkNav from '@/workspace/WorkNav'
import { useStudio, type Format } from '@/workspace/studioStore'
import { detectProduct, infographicIdea, hasAIKey } from '@/lib/ai'
...
const LOADER_MSGS = [
  ...
  'Генерируем виджеты инфографики…',
          ...
          <div key={s.n} className="flex items-center gap-2 sm:gap-4">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-full flex items-center justify-center transition-colors"
                style={{ background: active || done ? ACCENT : '#242424', color: active || done ? '#111' : 'rgba(255,255,255,0.5)', border: `1px solid ${active || done ? ACCENT : 'rgba(255,255,255,0.08)'}` }}>
                {done ? <Check size={17} strokeWidth={3} /> : <Icon size={17} />}
              </div>
              <span className={`text-sm font-semibold hidden sm:inline ${active ? 'text-white' : 'text-white/40'}`}>{s.label}</span>
            </div>
            {i < STEP_DEF.length - 1 && <div className="w-8 sm:w-14 h-0.5 rounded" style={{ background: s.n < step ? ACCENT : 'rgba(255,255,255,0.08)' }} />}
          </div>
        )
      })}
    </div>
...
export default function Infographics() {
  ...
  // нет выбранных инструментов → вернуть на выбор
  useEffect(() => { if (s.selectedTools.length === 0) navigate('/app') }, []) // eslint-disable-line

  const detect = async (dataUrl: string) => {
    if (!hasAIKey()) { s.setToast({ text: 'AI-ключ не задан — заполните поля вручную', icon: 'cpu' }); return }
    setDetecting(true)
    try {
      ...
      s.setToast({ text: 'AI распознал товар успешно!', icon: 'sparkles' })
    } catch { s.setToast({ text: 'Не удалось определить товар — заполните вручную', icon: 'cpu' }) } finally { setDetecting(false) }
  }

  const onProduct = (file?: File | null) => file && readFile(file, (url) => { s.setProductImage(url); detect(url) })
  const onRef = (file?: File | null) => file && readFile(file, (url) => s.setRefImage(url))

  const aiIdea = async () => {
    if (!hasAIKey()) { s.setToast({ text: 'AI-ключ не задан', icon: 'cpu' }); return }
    setIdeaLoading(true)
    try { const t = await infographicIdea(s.category, s.type); if (t) s.setInfographicText(t); s.setToast({ text: 'AI предложил текст преимуществ', icon: 'sparkles' }) }
    catch { s.setToast({ text: 'Не удалось сгенерировать идею', icon: 'cpu' }) } finally { setIdeaLoading(false) }
  }

  const compose = async (): Promise<string | null> => {
    const src = s.productImage
    if (!src) return null
    const ratio = RATIO[s.format]
    const long = RESPX[s.resolution]
    ...
    // title
    if (s.type) {
      ctx.fillStyle = '#fff'; ctx.font = `800 ${Math.round(34 * sc)}px Unbounded, Inter, sans-serif`
      ctx.shadowColor = 'rgba(0,0,0,0.6)'; ctx.shadowBlur = 8 * sc
      ctx.textBaseline = 'top'; ctx.fillText(s.type.slice(0, 26), pad, pad)
      ctx.shadowBlur = 0
    }
    // advantages
    const lines = s.infographicText.split('\n').map((l) => l.replace(/^[✦•\-\s]+/, '').trim()).filter(Boolean).slice(0, 5)
    const fs = Math.round(26 * sc); const lh = fs * 1.7
    let y = H - pad - lines.length * lh
    lines.forEach((line) => {
      ctx.fillStyle = ACCENT; ctx.beginPath(); ctx.arc(pad + fs * 0.35, y + fs * 0.6, fs * 0.28, 0, Math.PI * 2); ctx.fill()
      ctx.fillStyle = '#fff'; ctx.font = `600 ${fs}px Onest, Inter, sans-serif`; ctx.textBaseline = 'top'
      ctx.shadowColor = 'rgba(0,0,0,0.6)'; ctx.shadowBlur = 6 * sc
      ctx.fillText(line.slice(0, 40), pad + fs * 1.1, y + fs * 0.12)
      ctx.shadowBlur = 0
      y += lh
    })
  ...
  const runGeneration = async () => {
    setLoading(true); setProgress(0); setFeedback(null)
    let i = 0
    setLoaderMsg(LOADER_MSGS[0])
    const msgInt = setInterval(() => { i = Math.min(i + 1, LOADER_MSGS.length - 1); setLoaderMsg(LOADER_MSGS[i]) }, 700)
    const start = performance.now()
    const progInt = setInterval(() => setProgress(Math.min(100, ((performance.now() - start) / 3000) * 100)), 80)
    const composed = await compose().catch(() => null)
    setTimeout(async () => {
      clearInterval(msgInt); clearInterval(progInt); setProgress(100)
      s.setResultImage(composed)
      setLoading(false); setStep(3)
      s.setToast({ text: 'Инфографика готова!', icon: 'sparkles' })
    }, 3000)
  }

  const generate = () => { setStep(2.5 as number); runGeneration() }

  const startOver = () => {
    s.resetEditor(); setStep(1); setFeedback(null)
  ...
  const download = () => {
    ...
    const a = document.createElement('a'); a.href = s.resultImage; a.download = 'airoom-card.png'; a.click()
  }

  const step1Ready = !!s.productImage && !!s.refImage
        ...
        <div className="mt-10">
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}>
              {!s.productImage ? (
                <div onClick={() => productInput.current?.click()}
                  className="rounded-2xl border-2 border-dashed border-white/12 hover:border-[#F5C800]/50 transition-colors py-16 flex flex-col items-center text-center cursor-pointer">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {!s.refImage ? (
                    <div onClick={() => refInput.current?.click()} className="relative rounded-2xl border-2 border-dashed border-white/12 hover:border-[#F5C800]/50 transition-colors p-3 flex flex-col items-center justify-center cursor-pointer">
                      ...
                      <span className="mt-2 text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#F5C800]/15 text-[#F5C800]">Обязательно</span>
                    </div>
                  ) : (
                    <div className="relative rounded-2xl border border-white/[0.08] bg-[#242424] p-3">
              ...
              {step1Ready && (
                <div className="flex gap-3 mt-4">
                  {([['copy', 'Копировать стиль', Copy], ['inspire', 'Вдохновиться', Sparkles]] as const).map(([id, label, Icon]) => (
                    <button key={id} onClick={() => s.setStyleMode(id)}
              ...
              {/* AI category block */}
              {s.productImage && (
                <div className="mt-4 rounded-2xl border border-white/[0.08] bg-[#242424] p-5">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] text-white/45 mb-1.5">Категория товара</label>
                      <input value={s.category} onChange={(e) => s.setCategory(e.target.value)} placeholder={detecting ? 'Определяем…' : 'Категория'} className="w-full bg-[#2e2e2e] border border-white/10 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#F5C800]/50" />
                    </div>
                    <div>
                      <label className="block text-[11px] text-white/45 mb-1.5">Тип товара</label>
                      <input value={s.type} onChange={(e) => s.setType(e.target.value)} placeholder={detecting ? 'Определяем…' : 'Тип'} className="w-full bg-[#2e2e2e] border border-white/10 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#F5C800]/50" />
                    </div>
                  </div>
                  <p className="text-[12px] text-white/35 mt-3">AI определил категорию автоматически — можно отредактировать</p>
                </div>
              )}
          ...
          {step === 2 && (
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}>
              <div className="text-center mb-6">
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.02] px-3.5 py-1.5 text-[11px] tracking-wider text-white/55"><Cpu size={13} className="text-[#F5C800]" /> AI АНАЛИЗИРУЕТ ТОВАР</span>
                <h2 className="font-['Unbounded'] font-bold text-2xl sm:text-3xl mt-4">Параметры товара</h2>
                <p className="text-white/50 text-sm mt-2">AI определил детали — проверьте и отредактируйте</p>
              </div>

              {/* specs */}
              <div className="grid sm:grid-cols-2 gap-3">
                {s.specs.length === 0 && Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="rounded-xl bg-white/[0.04] h-[68px] animate-pulse" />
                ))}
                {s.specs.map((sp, i) => (
              ...
              <div className="mt-5 rounded-2xl border border-white/[0.08] bg-[#242424] p-5">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button onClick={aiIdea} disabled={ideaLoading} className="flex items-center gap-1 text-[12px] font-semibold text-[#F5C800] hover:underline disabled:opacity-50">
                    {ideaLoading ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} ✦ AI идея
                  </button>
                </div>
                <p className="text-[12px] text-white/40 mb-3">Напишите свободно, какой текст показать на карточке (преимущества товара)</p>
                <textarea value={s.infographicText} onChange={(e) => s.setInfographicText(e.target.value)} rows={4}
              ...
              <div className="mt-5 grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {FORMATS.map((f) => (
                      <button key={f} onClick={() => s.setFormat(f)} className="px-3 py-2 rounded-lg border text-[13px] font-semibold transition-colors"
                        style={{ borderColor: s.format === f ? ACCENT : 'rgba(255,255,255,0.1)', color: s.format === f ? ACCENT : 'rgba(255,255,255,0.6)' }}>{f}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex gap-2">
                    {RES.map((r) => (
                      <button key={r.id} onClick={() => s.setResolution(r.id)} className="relative px-4 py-2 rounded-lg border text-[13px] font-semibold transition-colors"
                        style={{ borderColor: s.resolution === r.id ? ACCENT : 'rgba(255,255,255,0.1)', color: s.resolution === r.id ? ACCENT : 'rgba(255,255,255,0.6)' }}>
                        {r.label}{r.id === '2k' && <span className="ml-1 text-[9px] text-white/35">реком.</span>}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex justify-between mt-6">
                ...
                <button onClick={generate} className="flex items-center gap-2 bg-[#F5C800] text-[#111] font-['Unbounded'] font-bold text-sm px-6 py-3.5 rounded-xl">Сгенерировать <Sparkles size={16} /></button>
              </div>
            </motion.div>
          )}
          ...
          {loading && (
            <div className="flex flex-col items-center text-center py-16">
              <Sparkles size={40} className="text-[#F5C800] animate-pulse mb-5" />
              <h3 className="font-['Unbounded'] font-bold text-xl mb-5">Генерируем вашу инфографику…</h3>
              <div className="w-full max-w-md h-1.5 rounded-full bg-white/10 overflow-hidden">
                <div className="h-full bg-[#F5C800] transition-all" style={{ width: `${progress}%` }} />
              </div>
              <div className="text-white/45 text-sm mt-4">{loaderMsg}</div>
            </div>
          )}
          ...
          {step === 3 && !loading && (
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
              <div className="text-center mb-6">
                ...
                <h2 className="font-['Unbounded'] font-bold text-2xl sm:text-3xl mt-4">Ваша инфографика готова</h2>
              </div>

              <div className="relative mx-auto rounded-2xl overflow-hidden border border-white/[0.08] bg-[#242424]" style={{ maxWidth: 440, aspectRatio: aspectCss }}>
              ...
              <div className="text-center mt-6">
                ...
                <div className="flex justify-center gap-3">
                  <button onClick={() => { setFeedback('like'); s.setToast({ text: 'Спасибо за оценку!', icon: 'thumbs-up' }) }}
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl border text-sm transition-colors"
                    style={{ borderColor: feedback === 'like' ? ACCENT : 'rgba(255,255,255,0.1)', color: feedback === 'like' ? ACCENT : 'rgba(255,255,255,0.7)' }}>
                    <ThumbsUp size={15} /> Отлично
                  </button>
                  <button onClick={() => { setStep(2.5 as number); runGeneration() }} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/10 text-white/70 text-sm hover:border-white/20"><ThumbsDown size={15} /> Переделать</button>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-8">
                <button onClick={download} className="flex items-center justify-center gap-2 bg-[#F5C800] text-[#111] font-['Unbounded'] font-bold text-sm py-3.5 rounded-xl"><Download size={16} /> Скачать карточку</button>
                <button onClick={() => { if (s.resultImage) { navigator.clipboard?.writeText(location.href); s.setToast({ text: 'Ссылка скопирована', icon: 'link-2' }) } }} className="flex items-center justify-center gap-2 border border-white/10 text-white/80 text-sm py-3.5 rounded-xl hover:border-white/20"><Share2 size={16} /> Поделиться</button>
    ...
    <AnimatePresence>
      {toast && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }}
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[1200] flex items-center gap-2.5 rounded-xl border border-white/[0.1] bg-[#242424] px-4 py-3 shadow-2xl">
